...
let handlePatientLogin = (email, password) => {
    return new Promise(async (resolve, reject) => {
        try {
            let userData = {};
            let isExist = await checkPatientName(email);

            if (isExist) {
                let result = await pool.query('SELECT * FROM patient WHERE email = $1', [email]);
                let user = result.rows[0];
                if (user) {
                    if (password === user.password) {
                        userData.errCode = 0;
...
module.exports = {
    handleUserLogin, handlePatientLogin
};
